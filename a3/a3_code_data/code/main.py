...
def q2_1():
    ...
    print(model.w)
    test_and_plot(
        model, X, y, title="WeightedLeastSquares", filename="weighted_least_squares_outliers.pdf"
    )
    """YOUR CODE FOR Q2.1"""
...
def q2_4_2():
    ...
    optimizer_gd = GradientDescent(max_evals=100, verbose=False)
    model_gd = LinearModel(fun_obj, optimizer_gd)

    # Fit the model with GradientDescent
    model_gd.fit(X, y)

    # Create a LinearModel with GradientDescentLineSearch optimizer
    optimizer_ls = GradientDescentLineSearch()
    model_ls = LinearModel(fun_obj, optimizer_ls)

    # Fit the model with GradientDescentLineSearch
    model_ls.fit(X, y)
   

    # Plot the learning curves for both optimizers
    plt.figure()
    plt.plot(range(len(model_gd.fs)), model_gd.fs, label="GradientDescent")
    plt.plot(range(len(model_ls.fs)), model_ls.fs, label="GradientDescentLineSearch")
    plt.xlabel("Number of Iterations")
    plt.ylabel("Objective Function Value")
    plt.legend()
    plt.title("Learning Curves for Robust Regression with Different Optimizers")
    plt.grid(True)
    plt.show()
