...
def q2_1():
    ...
    train_count = 0
    vali_count = 0
    # print(np.isnan(Y_train[0][0])==False)
    for i in range(n):
        for j in range(d):
            # print(Y_train[i][j])
            if np.isnan(Y_train[i][j]) == False:
                train_count = train_count + 1
            if np.isnan(Y_valid[i][j]) == False:
                vali_count = vali_count + 1
                
    print("train count :", train_count)
    print("validation count :", vali_count)
    
    non_nan_indices_train = np.argwhere(~np.isnan(Y_train))

    # Find the non-NaN indices in Y_valid
    non_nan_indices_valid = np.argwhere(~np.isnan(Y_valid))

    # Convert to sets of tuples for easier comparison
    set_indices_train = set(tuple(pair) for pair in non_nan_indices_train)
    set_indices_valid = set(tuple(pair) for pair in non_nan_indices_valid)

    # Check for intersection
    common_indices = set_indices_train.intersection(set_indices_valid)

    # Print the result
    if common_indices:
        print(f"Common indices found: {common_indices}")
    else:
        print("No common indices found between Y_train and Y_valid")
...
def q2_3():
    ...
    k = 50
    fun_obj_w = CollaborativeFilteringWLoss(lammyZ=10, lammyW=10)
    fun_obj_z = CollaborativeFilteringZLoss(lammyZ=10, lammyW=10)
...
def q3_2():
    ...
    fun_obj = MLPLogisticRegressionLossL2(encoder, predictor, 0.1)

    # Choose optimization strategy
    child_optimizer = GradientDescent()
    learning_rate_getter = ConstantLR(1e-2)
    # learning_rate_getter = InverseSqrtLR(1e-2)
    # learning_rate_getter = ConstantLR(1e-2)
