...
class CollaborativeFilteringZLoss(FunObj):
    ...
    def evaluate(self, z, W, X):
        ...
        k, _ = W.shape
        Z = z.reshape(n, k)
        print(" Z:", np.linalg.norm(Z, 'fro'))
        print("W :", np.linalg.norm(W, 'fro'))
        term1 = Z*self.lammyZ 
        term2 = W*self.lammyW 
        R = Z @ W - X 
        R[np.isnan(R)] = 0
        f = np.sum(R ** 2)/2 + np.sum(W **2 )*self.lammyW / 2 + np.sum( Z**2 ) * self.lammyZ / 2
        g = R @ W.T + term1
        return f, g.flatten()
...
class RobustPCAFeaturesLoss(FunObj):
    ...
    def evaluate(self, z, W, X):
        ...
        # R = np.abs(Z @ W - X)
        R = Z @ W - X
        f = np.sum((R ** 2 + self.epsilon)**(1/2))
        g = ( R / np.sqrt(R**2 + self.epsilon)) @ W.T
        return f, g.flatten()
...
class RobustPCAFactorsLoss(FunObj):
    ...
    def evaluate(self, w, Z, X):
        ...
        R = Z @ W - X
        f = np.sum((R ** 2 + self.epsilon)**(1/2))
        g = Z.T @ (R / np.sqrt(R**2+ self.epsilon))
        return f, g.flatten()
