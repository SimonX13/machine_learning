...
def logisticObj(w, X, y):
    yXw = y * (X @ w)
    f = np.sum(np.log(1 + np.exp(-y*X@w)))
    g = -X.T @ (y / (1 + np.exp(yXw)))
    return f, g
...
class MulticlassLinearClassifier(LinearClassifier):
    ...
    def fit(self, X, y):
        ...
        w_init = np.zeros(d*self.n_classes)
        w_on_sub, _ , _ ,_ = self.optimize(w_init, X, y)
        self.W = np.reshape( w_on_sub, (self.n_classes, d))
