...
def q5_1():
    ...
    # YOUR CODE HERE
    # print(X)
    print(np.min(X))    # 0.352
    print(np.max(X))    # 4.862
    print(np.mean(X))   # 1.3246250000000002
    print(np.median(X)) # 1.1589999999999998
    print(mode(X))
    #print(names)        # ['NE' 'MidAtl' 'ENCentral' 'WNCentral' 'SAt1' 'ESCentral' 'WSCentral' 'Mtn' 'Pac' 'WtdILI']
    ...
    indexMaxMean  = np.argmax(column_means)
    print(names[indexMaxMean])
    indexMinMean  = np.argmin(column_means)
    print(names[indexMinMean])
    
    column_variances = np.var(X, axis=0)
    indexMaxVar  = np.argmax(column_variances)
    print(names[indexMaxVar])
    indexMinVar  = np.argmin(column_variances)
    print(names[indexMinVar])
